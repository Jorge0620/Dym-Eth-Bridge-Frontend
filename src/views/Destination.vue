<template>
  <div class="text-center pt-12">
    <h1 class="text-2xl font-bold mb-8">
      Bridge from wDym({{destinationNetwork}}) to Dym({{originNetwork}})
    </h1>

    <p>
      This bridge allows you to send wrap token from wDym({{destinationNetwork}}) back to Dym({{originNetwork}})
    </p>
    ...
    <form class="w-96 mt-8 mx-auto">
      <label for="price" class="block mb-2 font-medium text-gray-700"
        >How much wrap token do you want to bridge?</label
      >
...
import { getT2CFeeAmount } from '../utils/actions'
...
export default defineComponent({
  ...
  setup() {
    ...
    const sendTokens = async function () {
      const feeAmount = await getT2CFeeAmount(amount.value)
      const amountFormatted = ethers.utils.parseUnits(amount.value, 18)
      console.log('amountFormatted :>> ', amountFormatted)
      console.log('amountFormatted.toString() :>> ', feeAmount.message.toString())
      ...
      if (typeof window.ethereum !== 'undefined') {
        ...
        try {
          const transaction = await contract.DepositToken(
